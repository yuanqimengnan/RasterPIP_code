...
void DataHandler::initData(QString polyIndex, QString indexFileStem, set<size_t> attIds, size_t locAtt) {
    ...
    std::cout<<"Init Polygons"<<t1.elapsed()<<"ms"<<endl;
    t1.restart();
    ...
    hashGridIndex = new HashGridIndex(indexFileStem, lx, ly, lz, p1, p2, p3);//fuzhieryi
    ...
    this->setAggregation();
    std::cout<<"Init Points"<<t1.elapsed()<<"ms"<<endl;
}
...
void DataHandler::executeQuery(float region_low[3], float region_high[3]) {
    ...
    experiment_time[0] = time;
    qDebug() << "Query Index - time taken:" << std::round(timer.nsecsElapsed() / 10000.0f) / 100.0f << "ms";
}
